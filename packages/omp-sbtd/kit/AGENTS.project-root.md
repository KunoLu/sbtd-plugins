<!-- KPi template: project-root; sourceId=sbtd-workflow-kit-upstream; revision=4222b15cc0e101bfe3489f1cebc0e5bfb4d1bddb; transform=p0-v3 -->
# Root Project Facts
...
- 陌生模块或上下文不清：代码阅读 / `codebase-design` → GitNexus exploring / impact-analysis → 主会话实现。
...
- 同时命中 legacy 和 refactoring gate 时，正常顺序为 `Legacy Change Safety Review` → `Refactoring Review` → 主会话实现；受控例外为 `seam-required` → `Refactoring Review` (`safety-seam-only`) → 建立 safety net → legacy `characterized` → 常规 `Refactoring Review`，避免测试 seam 死锁且禁止普通重构抢跑。
