...
When this Skill is actively used and the user's request includes either `sync` or `同步`, enter BDD Sync Mode. This mode syncs project `.feature` files with current product behavior; it does not mean local OMP extension or host configuration sync and does not copy this repository's templates to any global path.
