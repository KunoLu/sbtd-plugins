...
## OMP Dispatch and Channel Boundary

- Shared `.trellis/config.yaml`, `.trellis/workflow.md`, and task artifacts define workflow gates, not platform identity. The current host and generated integration decide execution: OMP projects use the generated `.omp/**` extension. Static inspection must not select a runtime.
- **OMP only, when the current host is OMP and `.omp/**` integration is available:** use the generated OMP `task` workers and `trellis-implement` / `trellis-check` agent definitions. Do not apply or infer dispatch settings or fallback modes from any other agent runtime; read the generated OMP extension and obey its workflow planning gate.
...
Where a generated workflow classifies its platform as sub-agent dispatch, `implement.jsonl` and `check.jsonl` must both contain real spec / research / task artifact entries before `task.py start` or dispatch begins; seed / `_example` rows are tolerated by runtime consumers for compatibility but are never planning-ready. For OMP, obey the generated workflow's planning gate: its extension may parse role-specific JSONL non-fatally, but that does not relax workflow readiness.
