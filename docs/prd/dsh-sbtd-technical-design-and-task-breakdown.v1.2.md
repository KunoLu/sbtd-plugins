...
| 文档版本 | 1.2 |
| 日期 | 2026-09-02 |
| 状态 | 已定方案，部分落地（T0 已在 main 7253b29 路径可安装；T1-T16 未实现；dsh-sbtd next 未发布） |
...
| 同仓兄弟 | `@kunolu/omp-sbtd` 0.1.0-rc.14（npm `next`；`latest` 仍是 GPL 时代的 0.1.0-rc.2） |
| 许可证 | Apache-2.0 |
...
## 0. v1.2 相对 v1.1 必须改的事实（2026-09-02 核对）

本节纠正 v1.1 已过时的事实（v1.0 行仍作历史）。**方案 2a 外壳插件不变**：不改 tools / hooks / Maestro 六步预检，也不改 T1–T16 依赖图（仅 T0 从「建仓」改为「让已有 stub 可安装」）。
...
- T1–T16 依赖图仍然有效。
- 不修改 640-skills。
- T0 已路径可安装；不要写从未 plugin-add。
- 不改根 README（next 是发布后文案）。
...
| T0 尚未开工 | stub 已在：noop `apply`、`cordis.patch.yml` id `sbtd`、`private: true`。T0 = 让 stub 可安装，不是新建仓库。v1.2：main 7253b29 已路径可安装，不要写从未 plugin-add |
...
| 许可证含糊 | `KPi/omp-sbtd` latest 仍是 GPL-3.0-only；`sbtd-plugins` 是 Apache-2.0；`omp-sbtd` `next` = 0.1.0-rc.14 |
...
- v1.2：仓内已有 .trellis/。不要 trellis init。不要优先私有 KPi。
- 不修改 640-skills。
- 不要自动 `trellis init`。
- onboard 或 trellis init 必须先经人工确认，默认不代执行。
...
- `@kunolu/omp-sbtd` 0.1.0-rc.14（npm `next`；`latest` 仍是 GPL 时代的 0.1.0-rc.2）
