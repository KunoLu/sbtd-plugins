# Archive — dsh-sbtd TODO through 2026-09-13

Archived from `docs/TODO.md` on 2026-09-13. Historical through #72 / #73 (`1c0903b`). **Not the live queue.** Live queue: `docs/TODO.md`.
...
| 上次同步 | 2026-09-13（上海） |
| 同步时 main | `578251b`（#72 @next pitfall → `578251b`；#70 rc.2 → `ae636e8`；#68 dsh CI → `c6c1d80`；#64 T16 → `2fe061d`；#65 FU4 → `2fc31b4`；#62 T15 → `0dbe135`；#61 gitignore；#59 T14 → `2b7cf61`；#57 T13 → `9289798`；#55 T12 → `61026a1`；#53 T11 → `d729d6e`；T10 #51 → `014f480`） |
...
| **P3** | T15–T16 入口 + 整包验收 | ✅ T15 ✅ #62 → `0dbe135`；T16 ✅ #64 → `2fe061d` |
...
| T16 | 端到端验收 | ✅ | #64 → `2fe061d`；scheme A finish；PASS 8/8；r2 CLEAN @ `97e365c`；REQUIRED_CHANGES=none；archive `09-11-dsh-sbtd-t16-e2e-acceptance-grill` + `09-11-dsh-sbtd-t16-e2e-acceptance-ddd`；host `@deepseek-ai/dsh@0.1.1-rc.2` |
...
| FU4 | clarify JSON schema host-compatible omit-null | ✅ | [#65](https://github.com/KunoLu/sbtd-plugins/pull/65) → `2fc31b4`；scheme A finish；CLEAN r3 @ `ca9eb8f`；tests 331/331；locks Q1B Q2B Q3C Q4A PASS；Q5A pending post-merge T16；R1 CLOSED；R2 CLOSED；REQUIRED_CHANGES=none；archive `09-12-dsh-sbtd-fu4-clarify-schema-grill` + `09-12-dsh-sbtd-fu4-clarify-schema-ddd`；host `@deepseek-ai/dsh@0.1.1-rc.2` |
...
| npm `@kunolu/dsh-sbtd@0.1.0-rc.2` → `next` | ✅ #70 → `ae636e8`；`npm publish --tag next`（`kuno_lu`）；smoke `next=0.1.0-rc.2`；`latest` 仍 `0.1.0-rc.1`；pack sha256 `95d199e435cf917cd2045e8ba0a7feb29a80fad16fd376f7df5101d408bfd0ed` |
...
| dsh @next pitfall | ✅ #72 → `578251b`；r1 CLEAN @ `615c5dd`；REQUIRED_CHANGES=none；archive `09-13-dsh-next-pitfall`；prefer explicit `@kunolu/dsh-sbtd@<version>`；`@next` can stick via profile pnpm lock；不 npm |
...
| 15 | dsh @next pitfall | ✅ #72 → `578251b` |

## 9. 变更日志（本文件）

| 日期 | 说明 |
|---|---|
| 2026-09-13 | dsh @next pitfall #72 → main `578251b`（TODO merge-SHA backfill） |
| 2026-09-13 | dsh @next pitfall #72 scheme A finish/merge：r1 CLEAN @ `615c5dd`；REQUIRED_CHANGES=none；squash `578251b`；不发布包 |
...
| 2026-09-12 | FU4 #65 → main `2fc31b4`（TODO merge-SHA backfill） |
| 2026-09-12 | FU4 #65 scheme A finish/merge：CLEAN r3 @ `ca9eb8f`；locks Q1B Q2B Q3C Q4A PASS；Q5A pending T16；REQUIRED_CHANGES=none；R1/R2 CLOSED；squash `2fc31b4`；不发布包；下一队列 T16 |
