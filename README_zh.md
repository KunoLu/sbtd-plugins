...
DSH 插件 —— T0 stub，宿主为 `@deepseek-ai/dsh` 0.1.1-rc.2（`latest` 与 `next` dist-tag 都是这个版本）。不要使用 0.1.2-alpha.4。使用 dist-tag `next` 添加，不要用裸包名。在包存在之前，直接复制粘贴 `plugin add` 命令会失败。
